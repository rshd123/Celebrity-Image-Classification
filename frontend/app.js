        ...
        $.post(url, {
            ...
            */
            ...
            if (match) {
                ...
                console.log("Class probabilities:", match.class_probability);
                
                for(let personName in classDictionary) {
                    let index = classDictionary[personName];
                    let proabilityScore = match.class_probability[index];
                    let elementName = "#score_" + personName;
                    console.log(`Trying to set ${elementName} to ${proabilityScore}`);
                    
                    // Check if element exists
                    let element = $(elementName);
                    console.log(`Element ${elementName} found:`, element.length > 0);
                    
                    if (element.length > 0) {
                        element.html(proabilityScore.toFixed(2));
                        console.log(`Successfully set ${elementName} to ${proabilityScore.toFixed(2)}`);
                    } else {
                        console.log(`ERROR: Element ${elementName} not found!`);
                    }
                }
            }
            // dz.removeFile(file);            
